...
- The maximum flow rate measured that year was `r signif(max(Q),3)` m^3^/s or `you fill it in here` mm/s.

## Identified hydrograph of 25 October 1997

I have plotted this hydrograph below.

```{r Q-plot2, echo=FALSE, eval=TRUE}
...
```{r}
HGstartdate<-as.POSIXct("1997-10-24 21:00:00")
HGenddate<-as.POSIXct("1997-10-26 00:00:00")
HG<-subset(Q,D>=HGstartdate & D<=HGenddate)
HGdates<-subset(D,D>=HGstartdate & D<=HGenddate)
```


I have decided that the beginning of the hydrograph occurred on `r format(HGstartdate,"%Y-%m-%d %H:%M") ` and ended on `fill in here`.
...
```{r Q-plot3, echo=FALSE, eval=TRUE}
...
par(mar=c(4.5,4.5,4,4.5))     # defines the sizes, in number of lines, for the margins (bottom, left, top, right)
    ...
    plot(D,C,col="orchid",type="l",cex=0.1,yaxt="n",     
